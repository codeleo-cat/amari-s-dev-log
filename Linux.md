...
---
- 파일 제거 - rm
- 자주 쓰이는 명령어를 쓰기 편하게 바꾸기 - alias
- 현재 프로세스들의 상태 표시 - ps
- (전체) 파일 시스템의 구조와 용량을 보여주기 - df (disk free)
- 파일 시스템의 하드 사용량을 체크 - du
- 현재 시스템에 login 중인 사용자의 list를 보여줌 - who
- 파일과 디렉토리의 사용 권한을 부여 - chmod
- 명령어 x에 대한 매뉴얼 페이지 얻기 - man
- 프로세스를 중지 - kill