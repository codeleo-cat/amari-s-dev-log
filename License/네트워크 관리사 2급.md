...
- 수신측 TCP는 수신받은 패킷을 재조립하여 에러가 있으면 송신측에 재전송을 요구한다.
...
- ICMP, IGMP(Multicast), RARP 는 모두 전송 계층 프로토콜 (layer 4)
- 네트워크 장비 관리 감시 - SNMP
- Hop의 총계만 사용하는 - RIP ?
...
- Swap Space ?
- Active Directory 도메인 개체에 접근했을 때 기록이 남도록 감사 정책을 설정 - 시스템에 바로 적용하기 위한 명령어
- local host에서 도메인 쿼리를 어느 NS에 질의할 것이지 결정해주는 파일. domain resolution - 
