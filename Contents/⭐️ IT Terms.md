...
- **하나의 컴퓨터에서 실행 중인 프로세스들이 서로 겹치지 않도록 고유하게 할당되는 값**입니다.
- 전송 계층에서 사용되는 기술
- sender가 data를 보낼 때 **+** 수신자 컴퓨터의 process의 port 번호

1. **포트 번호는 고유하게 할당됨**:
• **포트 번호**는 하나의 컴퓨터에서 실행 중인 프로세스들이 **네트워크 트래픽을 구분**하기 위해 사용하는 고유한 값입니다. 각 프로세스는 특정 포트 번호에 바인딩되어, 그 포트로 들어오는 요청을 처리합니다.
• 포트 번호는 프로세스들이 네트워크 통신을 할 때 충돌하지 않도록 **고유하게 할당**되어야 합니다.

2. **한 프로세스가 여러 포트를 사용할 수 있음**:
• **하나의 프로세스가 여러 개의 포트 번호**를 가질 수 있습니다. 예를 들어, 웹 서버 프로세스는 **포트 80**(HTTP)과 **포트 443**(HTTPS)에서 동시에 요청을 수신할 수 있습니다.
• 각 포트 번호는 고유해야 하지만, 한 프로세스가 여러 소켓을 열어 각각의 포트 번호에 바인딩할 수 있습니다.

3. **동일한 프로토콜에서는 동일한 포트 번호를 공유할 수 없음**:
• **동일한 프로토콜(TCP/UDP)**에서 동일한 포트 번호는 **동시에 두 개 이상의 프로세스에서 사용할 수 없습니다**. 예를 들어, A 프로세스가 **TCP 포트 80**을 사용 중이라면, B 프로세스는 같은 **TCP 포트 80**을 사용할 수 없습니다.
• 그러나 **다른 프로토콜**(예: **TCP 80**과 **UDP 80**)은 동시에 사용할 수 있습니다. 각각은 다른 프로토콜 스택을 사용하기 때문에 서로 간섭하지 않습니다.

#process ✅ 실행 중인 프로그램
- 프로그램이 실행되어서 메모리에 올라간 상태
- **프로세스의 구성 요소**
	• **코드(CODE)**: 실행할 명령어들이 저장된 부분.
	• **데이터(DATA)**: 전역 변수, 정적 변수 등이 저장된 부분.
	• **스택(Stack)**: 함수 호출 시의 지역 변수, 리턴 주소 등을 저장하는 부분.
